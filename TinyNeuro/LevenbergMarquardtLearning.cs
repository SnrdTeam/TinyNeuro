    ...
    public class LevenbergMarquardtLearning : ISupervisedLearning
    {
        ...
        public double RunEpoch(double[][] input, double[][] output)
        {
            ...
            for (int s = 0; s <= Blocks; s++)
            {
                ...
                int[] block = Range(s * blockSize, s * blockSize + B);

                double[][] inputBlock = GetFromArray(input, block);
                double[][] outputBlock = GetFromArray(output, block);
        ...
        private static T[][] GetFromArray<T>(T[][] source, int[] indexes, bool transpose = false)
        {
            if (source == null)
